...
// JSON file with all questions (PNG URLs)
const dataPath = path.resolve('./src/assets/questions/algebra/questions');
...
interface Question {
  ...
  questionPngUrl: string;   // <-- PNG path served from /public
  solutionPngUrl: string;   // <-- PNG path served from /public
  tags: string[];
  complete: boolean;
}
...
export async function get({ url }: APIContext) {
  try {
    ...
    const payload = {
      ...chosen,
      // add seconds for frontend timer convenience
